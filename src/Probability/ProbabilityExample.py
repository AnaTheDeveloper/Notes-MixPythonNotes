...
# In data science, probability calculations are used to build models. Models are able to help us understand data that
# has yet to exist - either data we hadn’t previously collected or data that has yet to be created. Data scientists
# create models to help calculate the probability of a certain action and then they use that probability to make
# informed decisions.
...
num_of_people_in_a_room = 10

# Simulated room with a certain number of people
...
def calculate_probability(num_people):
    ...
    if num_people < 2:
        ...
        print(
            "\n\nThe probability that two people in a room of {0} people have the same birthday is nearly {1}%".format(
                num_people, rounded))
...
simulate(num_of_people_in_a_room)